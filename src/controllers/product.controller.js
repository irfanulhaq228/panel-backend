...
const createProduct = catchAsync(async (req, res) => {
  const { productName, productCode, productDescription, productPrice, productDiscount } = req.body;
  ...
  res.status(200).json({success:true,message:"Product Created Succfully" , product});
});
...
const updateProduct = catchAsync(async (req, res) => {
  ...
  // Handle images array in request body
  let images = req.body.images || [];
  if (typeof images === 'string') {
    // If the images field is a string, parse it
  ...
  // Update the product with new image list and other data
